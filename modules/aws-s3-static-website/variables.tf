...
/*variable "index_document_suffix" {
  description = "Suffix for index documents to render."
  type        = string
  default     = "index.html"
}

variable "error_document_key" {
  description = "Key for error documents to render."
  type        = string
  default     = "error.html"
}
...
variable "files" {
  ...
  type = object({
    terraform_managed     = bool    // Object's attributes without default value -> If the module is used -> REQUIRED to pass it
    error_document_key    = optional(string, "error.html")
    index_document_suffix = optional(string, "index.html")
    www_path              = optional(string)  # If user of this module does NOT specify it -> null
  })
}
